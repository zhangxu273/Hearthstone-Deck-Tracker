...
namespace HDT.Core.Tests
{
	...
	public class GameStateModifierTests
	{
		...
		public void TestFullEntity()
		{
			...
			var entityData = new EntityData(5, null, "NEW1_034", Zone.PLAY, EntityType.CardEntity);
			gameState.Apply(new FullEntity(entityData));
			Assert.AreEqual(gameState.Entities.Count, 1);
			var entityData2 = new EntityData(7, null, "NEW1_033", Zone.HAND, EntityType.CardEntity);
		...
		{
			var gameState = new GameState();
			var entityData = new EntityData(5, null, null, null, null);
			gameState.Apply(new FullEntity(entityData));
			var entity = gameState.Entities[5];
